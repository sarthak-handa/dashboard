...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (user && user.name) {
        document.getElementById("engineerNameDisplay").textContent = user.name;
        // document.getElementById("engineerNameDisplay").textContent =
        //   "LOCAL (" + user.name + ")";
    } else if (user && user.email) {
        document.getElementById("engineerNameDisplay").textContent = user.email;
        // document.getElementById("engineerNameDisplay").textContent =
  ...
  `;
    ...
    if (user) {
        ...
        // Department-based access (independent of the account role). HR and
        // Director users may open certain admin tools (e.g. Attendance Logs)
        // even though their account role is a plain "user". Branch names are
        // matched case-insensitively.
        const userBranches = user.branch || user.branches || [];
        const branchName = (b) => ((b && (b.name || b)) || "").toString().toLowerCase();
        const isHrOrDirector = userBranches.some((b) => {
        ...
        // Directors only (no HR) — the Activity Log is a Superadmin/Director tool.
        const isDirector = userBranches.some((b) => branchName(b).includes("director"));
        ...
        const menuItems = [
            ...
            },
            {
                ...
                // Also visible to HR / Director department users regardless of their
                // account role (backend enforces the same access on /attendance).
                allow: () => isHrOrDirector,
            ...
            },
            {
                ...
                // Also visible to Director-department users regardless of their
                // account role (backend enforces the same access on /activity).
                allow: () => isDirector,
        ...
        const adminOnlyItems = menuItems.filter(
            (item) =>
                !item.roles.includes("user") && !item.roles.includes("customer"),
        );
        ...
        // An item is shown when the role+branch match, OR when it carries an
        // `allow(user)` predicate that returns true (e.g. HR/Director access).
        const itemVisible = (item) =>
            (item.roles.includes(role) && item.branches.includes(branch)) ||
            (typeof item.allow === "function" && item.allow());

        const hasAdminItems = adminOnlyItems.some((item) => itemVisible(item));
            ...
            adminOnlyItems
                ...
                .forEach((item) => {
                    if (itemVisible(item)) {
                        ...
                        li.innerHTML = `<a class="dropdown-item" href="${item.href
                            }" target="${item.target || "_self"}">${item.label}</a>`;
